...
Another important thing about Elliptic Curve in cryptography is that is must be non-singular, like no sharp point, self intersect and stuff. It is to ensure that the operartions on it and its points can be done without problems. We will get to knows its operations right after this. But for now, an Elliptic curve have to satisfy Δ is non-zero with:
$$
Δ=−16(4a^3+27b^2)
$$
...
Points on an elliptic curve are all pair (x, y) satisfy the according elliptic function, and an "infinity point" denoted $O$, serves as the identity element. The number of points on a curve(including $O$) is called "The order of the curve".
Elliptic curves are also symmetric to the $x$ axis. Which means that if the point $𝑃 = (𝑥, 𝑦)$ is on the curve, then the point $−𝑃 = (𝑥, −𝑦)$ is also on the curve, and they are each other's "inverse". 

### Point addition

In an elliptic curve, you can take two points on it and perform an "add" to get a third point. Adding on ECC is kinda different from the usual add we know so we will denote it by the symbol ⊕ (or you can just use normal "+" just pick what suits you). 
...
How about adding a point to itself? Adding a point $P$ to itself means $P$ = $Q$, or the line $L$ is the tangent line to E at $P$. The procedure is the same, line $L$ intersect with E at $P$, $P$ and point $R$. Sum of $P$ and $P$ is $-R$. From this, we have "multipy" operation which is repeated addition:
$$
nP = P+P+P+...+P
$$
...
Well, points on an Elliptic Curves forms an Albelian group, since its addition law satisfies Group's properties:
...
The associative law is more complicated to proof. There is a proof in "Rational Points on Elliptic Curves", books by Silverman and Tate. Its a bit long so I will left a link here if you want to know the proof. Now we know all points on an elliptic curve forms a group. It should have all group's properties
...
- The ECQV implicit certificate schem
...
When both parties agree on an Elliptic Curve E and a generator point P ∈ E.
...
sequenceDiagram
    participant Alice
    participant Bob
    %Note over Victim: Select a random private key a
    %Note over Attacker: Select a random private key b
    Note over Alice,Bob: Select a random private key<br> Alice:k_a<br>Bob:k_b
    %Alice->>Bob: PK (public key)
    Note over Alice,Bob: Compute the Public key:<br>Alice:Q_a=k_aG<br>Bob:Q_b=k_bG
    Alice->>Bob: Qa ∈ E  
    Bob->>Alice: Qb ∈ E

    Note over Alice,Bob: Compute the shared secret:<br/>Alice:X = k_aQ_b = k_ak_bG<br/>Bob:X = k_bQ_a = k_ak_bG
    %Note over Bob: Compute the shared secret<br/> DHKey = [SK]Q₁
    %Alice->>Bob: C = E_DHKey(M)
```
...
1.  Alice generates a random private key $d_A$ and computes her public key $Q_A = d_A G$.
...
4.  She takes the x-coordinate of point $R$ (let's call it $r$) and calculates $s = k^{-1}(h + r d_A) \pmod n$.
...
2.  He computes $u_1 = h s^{-1} \pmod n$ and $u_2 = r s^{-1} \pmod n$.
3.  He calculates the point $P = u_1 G + u_2 Q_A$.
...
$P = (h s^{-1}) G + (r s^{-1}) Q_A$
And substituting $Q_A$ with $d_A G$:
$P = (h s^{-1}) G + (r s^{-1}) (d_A G)$
$P = (h s^{-1} + r d_A s^{-1}) G$
$P = ((h + r d_A) s^{-1}) G$
From the signing step, we know that $s = k^{-1}(h + r d_A)$. This means $s^{-1} = (h + r d_A)^{-1} k$.
Substituting $s^{-1}$ back into the equation for $P$:
$P = ((h + r d_A) (h + r d_A)^{-1} k) G$
$P = kG$
...
```
By observing the output, the attacker can recover the victim’s secret key modulo p~1~, and repeat the process with different curves to fully recover the key using the Chinese Remainder Theorem.
 -**Singular Curve Attack**
 **A special case of Invalid Curve Attack**
 In ECC, a curve is considered valid only if its discriminant 
 $$
 Δ =−16(4a^3+27b^2) ≠ 0 \pmod p
 $$
 If **Δ = 0 (mod p)**, the curve is singular, meaning it contains cusps or self-intersections, and the group law breaks down. The Elliptic Curve can be written as:
 $$
...
**Example**
-Suppose Alice selects a secret key $n_a \in \{1,2,3,...,104\}$ and computes her public key:
$$
A=n_a \cdot G
$$
...
-For each prime divisors $q \in \{3,5,7\}$, set
$$
h= \frac{ord(G)}{q}
$$
-For example, when $q=3$, we have $h=\displaystyle\frac{105}{3}$. Define
$$
\displaylines{
G' = h \cdot G \\
A' = h \cdot A = h \cdot (n_a \cdot G) = n_a \cdot(h \cdot G)
}
$$
...
$$
n_a\equiv k_3 \pmod3, n_a\equiv k_5 \pmod5, n_a\equiv k_7 \pmod7
$$
...
-Let say we have a polynomial $f(X) \in \mathbb{Z}[X]$ and we know a $x$ such that $f(x)\equiv0 \pmod{p^s}$ and we want to find $x' \in \mathbb Z$ which satisfy both $$f(x')\equiv0 \pmod{p^{s+1}} \text{ and } x'\equiv x \pmod{p^s}$$
Let $$x'=x + \delta\space \rightarrow \delta \equiv 0 \pmod{p^s} $$
Using a Taylor expansion of $f$ around $x$:
$$
f(x')=f(x+\delta)=f(x) + f'(x)\cdot \delta + \frac{f''(x)}{2!}\cdot \delta^2 +  \frac{f'''(x)}{3!}\cdot \delta^3 +\space...
$$
Reducing modulo $p^{s+1}$, note that $\delta \equiv 0 \pmod{p^s}$ implies $\delta^2 \equiv 0 \pmod{p^{2s}}$
$$
f(x')=f(x+\delta) \equiv f(x) + f'(x)\cdot \delta \pmod{p^{s+1}}
$$
Remind that $f(x') \equiv 0 \pmod{p^{s+1}}$, we assume $f'(x)$ is invertible and $u \cdot f'(x) \equiv 1 \pmod{p^{s+1}}$.
$$
\displaylines{
f(x) + f'(x)\cdot \delta \equiv 0 \pmod{p^{s+1}} \\
\rightarrow \delta \equiv\displaystyle\frac{-f(x)}{f'(x)} \pmod{p^{s+1}} \\
\delta \equiv- u \cdot f(x) \pmod{p^{s+1}} \text{   (1)}
}
$$
Finally substitute (1) in $x' = x + \delta$.
$$
x'=x + \delta \equiv x -u \cdot f(x) \pmod{p^{s+1}} \text{ (2)}
$$
Relation (2) is the **key step** of **Hensel’s lemma**: it shows how a root modulo $p$ can be uniquely extended (or “lifted”) to a root modulo $p^2$. Here $x'$ is referred to as a lift of $x$ modulo $p^{s+1}$. We will be using this theorem to lift element from $\mathbb{F}_p$ to the field of $p$-adic numbers $\mathbb{Q}_p$ described next.
**P-adic numbers**
-A $p$-adic number can be represent as:
$$
s=\sum_{i=k}^{\infty} a_i p^i=a_k\cdot p^k + a_{k+1}\cdot p^{k+1} + a_{k+2}\cdot p^{k+2} + ...
$$
Where $k$ is an integer (possibly negative), and each $a_i$ is an integer such that $0\leq a_i<p$. A p-adic integer is a $p$-adic number such that $k \geq 0$.
Note that $p$-adic number is similar to base-$p$, but instead of extending to the right, it extend to the left!
**For example**, convert $\displaystyle\frac{1}{5}$ in **base 3 and 3-adic:**
$$
\displaylines{
\displaystyle\frac{1}{5}=0.01210121... \text{(base 3)}=0\cdot3^0+0\cdot3^{-1}+1\cdot3^{-2}+2\cdot3^{-3}+1\cdot3^{-4}+... \\
\displaystyle\frac{1}{5}=...121012102 \text{(3-adic)}=...0\cdot3^{5}+1\cdot3^{4}+2\cdot3^{3}+1\cdot3^{2}+0\cdot3^{1}+2\cdot3^{0}.
}
$$
Wait isn't **3-adic** a bit weird $...+2\cdot3^{3}+1\cdot3^{2}+0\cdot3^{1}+2\cdot3^{0}$ *must* be **greater** than $\displaystyle\frac{1}{5}$. How could it be that way?
Well first, let talk about $p$-adic properties, we can do multiplication, addition and subtration just like how we do it with normal numbers.
For example, in **10-adic**(I know 10 is not a prime but we'll talk abt it later), we can do like this

| Addition | Multiplication | Subtraction |
|----------:|----------------:|----------:|
| $\space \text{ } \space...142857142857142$ <br>$+...345678345678345$<br> |$\space \text{ } \space...857142857143$ <br>$\displaystyle\times7$<br>|$\text{ }\space \text{ }...987654321$<br> $-...876543210$
$\space \text{ } \space ...488535488535487$|$...000000000001$|$...111111111$
...
0.999999999...=1
$$
The most straight-forward way to prove it is letting that number $0.999999999...=k$. Then multiply $k$ by 10 and subtract by $k$ itself:
$$
\displaylines{
0.999999999...=k \\
9.999999999...=10k \\
\rightarrow 9.999999999... -\space 0.999999999...=9k \\
\rightarrow 9=9k\text{, } k=1
}
$$
...
...999999999=m \\
...999999990=10m \\
\rightarrow ...999999999 -\space ...999999990=-9m \\
\rightarrow 9=-9m\text{, } m=-1
}
$$
At first, it didn't make any sense that $...999999999=-1$ at all. But when we add that number by $1$:
$$
\displaylines{
\text{ } \space...999999999 \\
+1 \\
\hline
\space \text{ } \space ...000000000 \\=0
}
$$
...
$$
...nnnnnn= n\cdot p^0 + n\cdot p^1 + n\cdot p^2 + n\cdot p^3 = \displaystyle\frac{n}{1-p}
$$
For example $2$ in $3$-adic
$$
...222222222=\displaystyle\frac{2}{1-3} = -1
$$
...
P(x,y)
$$
Instead of $x$, $y$ in $\mathbb Z$
We are converting them to $p$-adic numbers!
$$
\displaylines{
x=x_0 + x_1 \cdot p + x_2 \cdot p^2 + x_3 \cdot p^3 + x_4 \cdot p^4 +\space ... \\
y=y_0 + y_1 \cdot p + y_2 \cdot p^2 + y_3 \cdot p^3 + y_4 \cdot p^4 +\space ... \\
}
$$
So a point $P$ in Elliptic Curve over $\mathbb Q_p$ is now
$$
P(x_0 + x_1 \cdot p + x_2 \cdot p^2 +\space...,y_0 + y_1 \cdot p + y_2 \cdot p^2 +\space...) = P(x_0,y_0)
$$
About the curve coefficients when lifting
When we lift the curve itself from $\mathbb F_p$ to $\mathbb Z/ p^2\mathbb Z$, the coeficients also need to be lifted. Formally, one could choose any:
$$
A' = A + \alpha p ,\space B'= B + \beta p
$$
...
However, for Smart’s attack we don’t actually need exotic choices of $(A', B')$. It is enough to take the **canonical lift**, i.e. simply reuse the same coefficients $A'=A,\space B'=B$. This keeps the equation consistent and makes the exposition simpler.
...
$$
\displaylines{
\psi(P)=\int_{\infty}^{P}\frac{dx}{2y}=s(P)+\displaystyle\frac{a_1}{2} \cdot s(P)^2 + \left(\displaystyle\frac{a_1^2}{3} + \displaystyle\frac{a_2}{3}\right) \cdot s(P)^3 +\space ... \\
\text{where }a_1{,}a_2{,}a_3\text{ are coefficients from a generalized Weierstrass equation}\\
...
$$
\displaylines{
\psi(P)=s(P)+0 \cdot s(P)^2 + 0\cdot s(P)^3 +\space ... = s(P)
}
$$
Therefore, $\psi(P)$ is now just $s(P)$.
**The Attack**
The attack proceeds by lifting the points $P$ and $Q = kP$ from $E(\mathbb F_p)$ to their canonical lifts $P'$ and $Q'$ in $E(\mathbb Q_p)$—that is, the unique lifts that lie in the formal group ($s(P') ∈ p\mathbb Z_p$).
Thanks to the homomorphism property and the simplification, we get:
$$
s(Q')=k \cdot s(P')
$$
But wait, why do we have to lift to $P'$, $Q'$?
Why not just calculating $$s(Q)=k \cdot s(P)$$
directly in $\mathbb F_p$? Couldn’t we simply compute
$$s(P) = -\displaystyle\frac{x(P)}{y(P)}\text{ and } s(Q) = -\displaystyle\frac{x(Q)_0}{y(Q)_0}$$ modulo $p$ and solve $s(Q) ≡ k \cdot s(P) \pmod p$?
...
$$
s(P) = \displaystyle\frac{x_0}{y_0} \not\equiv 0 \pmod p
$$
So the $\psi(P)$ can't be approximated correctly at the level of $\mathbb F_p$.
This is why we lift. By lifting $P$ to $P'$ over $\mathbb Z/\mathbb p^2Z$, and then multiplying by $p$, the resulting point $pP'$ does land in the formal group (since its $s$-coordinate is divisible by $p$). At that point, the $p$-adic logarithm is well-defined, and its linearizing property applies.
Hence $k$ can be solve as:
$$
\displaylines{
s(pQ')=k\cdot s(pP')\\
\rightarrow k \equiv \displaystyle\frac{s(pQ')}{s(pP')} = \displaystyle\frac{p\cdot s(Q')}{p\cdot s(P')}\pmod {p^2}\\
\rightarrow k \equiv \displaystyle\frac{s(Q')}{s(P')} \pmod p
}
$$
Before solving for $k$, we need to know coordinates of $P'$ and $Q'$ which is finding $x_1(Q),\space x_1(P),\space y_1(Q),\space y_1(P)$. Well it doesn't really matter at all, any point $P'$ can be multiply by $p$ in order to apply logarithm, so $x_1$ can be picked as any numbers like $0$ then lift_x to find $y_1$ lol
...
Let $n$ be a large prime that divides the number of rational points on the curve, denoted as $\#E(\mathbb{F}_q)$.

A **Pairing** (either a Weil or Tate Pairing) is a bilinear map:

$$
e_n : E[n] \times E[n] \longrightarrow \mu_n \subset \mathbb{F}_{q^k}^{\times}
$$
(briefed, from https://crypto.stanford.edu/pbc/notes/elliptic/weil.html)
...
* **$\mu_n$** is the group of the $n$-th roots of unity, located within the extension field $\mathbb{F}_{q^k}$.
...
The original problem is now reduced to a discrete logarithm problem in the finite field $\mathbb{F}_{q^k}$. We must find the exponent $r$ in the equation $v = u^r$.

This attack is viable for some curves with the embedding degree small enough (specially supersingular curves, where k<=6) and can be simply avoided by not using curves with small embedding degree.
